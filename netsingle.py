...
def print_conversation_header(pkt):  
    try:
       if(pkt.ip.dst==sys.argv[1] or pkt.ip.src==sys.argv[1]):
           ...
           if pkt.ip.src==sys.argv[1]:
            #print(str(pkt.http))
            fing=open(sys.argv[1]+"data.txt",'a')
            fing.write(urllib.parse.unquote(str(pkt.http).splitlines()[1])+"\n")
           if tme%2==0:
            if readFromFile()==2:
